...
- `mars sync` now writes dependency aliases to `.mars/models-dependencies.json` (dependency-only snapshot) and no longer persists builtin fallback aliases into on-disk alias state.
