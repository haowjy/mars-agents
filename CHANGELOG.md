...
### Changed
- `mars unlink` now removes only outputs and config entries recorded as owned
  for that target; handwritten files remain in place, and the target directory
  remains when it is not empty.
