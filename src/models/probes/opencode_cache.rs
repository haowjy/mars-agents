...
///
/// This helper is read-only and never triggers a probe refresh.
pub fn read_cached_probe_result_usable() -> Option<OpenCodeProbeResult> {
    let entry = read_cache_tolerant()?;
    if !is_usable(&entry) {
        return None;
    }
...
mod tests {
    ...
    #[test]
    fn stale_entry_with_synchronous_refresh_runs_probe_without_spawn() {
        let temp = TempDir::new().unwrap();
        let path = cache_file(&temp);
        ...
        let outcome = probe_cached_impl(
            ...
            || {
                ...
                OpenCodeProbeResult {
                    model_slugs: vec!["openai/gpt-5.5".to_string()],
                    model_probe_success: true,
                    error: None,
                }
            },
