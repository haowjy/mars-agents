//! Unified frontmatter reference rewriting after renames.
//!
//! Explicit config renames and automatic collision renames are indexed together
//! before any agent content is rewritten. Each agent gets one rewrite pass that
//! resolves skill and subagent references against the post-prune target state,
//! so guards such as "same source still owns the unrenamed item" see only items
...
/// All installed-name rewrites indexed by original item name.
#[derive(Debug, Default)]
pub struct RenameIndex {
    /// Skill original_name -> [(installed_name, source_name)].
    skill_renames: HashMap<ItemName, Vec<(ItemName, SourceName)>>,
    /// Subagent original_name -> [(installed_name, source_name)].
    subagent_renames: HashMap<ItemName, Vec<(ItemName, SourceName)>>,
}
...
impl RenameIndex {
    pub fn new(
        ...
        let mut skill_collision_final_names: HashMap<(SourceName, ItemName), ItemName> =
            HashMap::new();

        for rename in collision_renames {
            if !target_has_item(target, &rename.source_name, rename.kind, &rename.new_name) {
                continue;
            }

            let renames = match rename.kind {
                ItemKind::Skill => {
                    skill_collision_final_names.insert(
                        (rename.source_name.clone(), rename.original_name.clone()),
                        rename.new_name.clone(),
                    );
                    &mut index.skill_renames
                }
                ItemKind::Agent => &mut index.subagent_renames,
                _ => continue,
            };
            push_rename(
                renames,
                ...
                rename.new_name.clone(),
                rename.source_name.clone(),
            );
        }

        for rename in explicit_skill_renames {
            let installed_name = skill_collision_final_names
                .get(&(rename.source_name.clone(), rename.new_name.clone()))
                .unwrap_or(&rename.new_name);
            if !target_has_item(target, &rename.source_name, ItemKind::Skill, installed_name) {
                continue;
            }
            push_rename(
                &mut index.skill_renames,
    ...
    pub fn is_empty(&self) -> bool {
        self.skill_renames.is_empty() && self.subagent_renames.is_empty()
    }
}

/// Apply explicit and collision renames to each agent in one content update.
pub fn apply_renames(
    target: &mut TargetState,
    index: &RenameIndex,
    graph: &ResolvedGraph,
    dep_precedence: &[SourceName],
    ...
    for key in agent_keys {
        ...
        let agent_deps = ordered_agent_deps(graph, &source_name, dep_precedence);
        let skill_renames = renames_for_agent(
            target,
            &source_name,
            &agent_deps,
            ItemKind::Skill,
        ...
        let subagent_renames = renames_for_agent(
            target,
            &source_name,
            &agent_deps,
            ItemKind::Agent,
            &index.subagent_renames,
        );
        if skill_renames.is_empty() && subagent_renames.is_empty() {
            continue;
        }

        let mut rewritten_content = content;
        let mut changed = false;
        rewrite_content_for_agent(
        ...
        if changed && let Some(target_item) = target.items.get_mut(&key) {
            target_item.rewritten_content = Some(rewritten_content);
        }
    }
...
fn push_rename(
    renames: &mut HashMap<ItemName, Vec<(ItemName, SourceName)>>,
    original_name: ItemName,
    new_name: ItemName,
    source_name: SourceName,
) {
    let entries = renames.entry(original_name).or_default();
    if entries.iter().any(|(existing_name, existing_source)| {
        existing_name == &new_name && existing_source == &source_name
    }) {
        return;
    }
    entries.push((new_name, source_name));
}
...
fn ordered_agent_deps(
    graph: &ResolvedGraph,
    source_name: &SourceName,
    dep_precedence: &[SourceName],
) -> Vec<SourceName> {
    if source_name.as_str() == "_self" {
        return dep_precedence.to_vec();
    }
...
fn renames_for_agent(
    target: &TargetState,
    source_name: &SourceName,
    agent_deps: &[SourceName],
    ...
    for (original_name, entries) in renames {
            ...
            && source_has_unrenamed_item(target, source_name, referenced_kind, original_name)
        {
            ...
            selected.or_else(|| {
                agent_deps
                    .iter()
                    .find_map(|dep| entries.iter().find(|(_, source)| source == dep))
            })
        };
...
fn rewrite_content_for_agent(
    ...
    match rewrite_content(content, renames) {
        Ok(Some(new_content)) => {
            *content = new_content;
            *changed = true;
        }
...
mod tests {
    ...
    fn apply_test_renames(
        ...
        let index = RenameIndex::new(explicit_skill_renames, collision_renames, target);
        apply_renames(target, &index, graph, dep_precedence).unwrap();
    }

    #[test]
    fn apply_renames_uses_exact_skill_matches() {
    ...
    #[test]
    fn apply_renames_leaves_non_matching_agents_unchanged() {
        let dir = TempDir::new().unwrap();
        let agent_path = dir.path().join("agents/coder.md");
        fs::create_dir_all(agent_path.parent().unwrap()).unwrap();
        fs::write(&agent_path, "---\nskills: [review]\n---\n# Agent\n").unwrap();
    ...
    #[test]
    fn apply_renames_cross_package_uses_dep_graph() {
        let dir = TempDir::new().unwrap();
        let agent_path = dir.path().join("agents/coder.md");
        fs::create_dir_all(agent_path.parent().unwrap()).unwrap();
        fs::write(&agent_path, "---\nskills:\n- planning\n---\n# Agent\n").unwrap();
    ...
    #[test]
    fn local_agent_uses_config_dependency_order_not_graph_order() {
        let dir = TempDir::new().unwrap();
        let agent_path = dir.path().join("project/.mars-src/agents/coder.md");
        fs::create_dir_all(agent_path.parent().unwrap()).unwrap();
        fs::write(&agent_path, "---\nskills: [planning]\n---\n# Local Agent\n").unwrap();
    ...
    #[test]
    fn dependency_agent_uses_config_dependency_order_for_renamed_refs() {
        let dir = TempDir::new().unwrap();
        let agent_path = dir.path().join("source-a/agents/coder.md");
        fs::create_dir_all(agent_path.parent().unwrap()).unwrap();
        fs::write(&agent_path, "---\nskills: [planning]\n---\n# Agent\n").unwrap();
        ...
        let skill_b_path = dir.path().join("source-b/skills/planning");
        let skill_c_path = dir.path().join("source-c/skills/planning");
        fs::create_dir_all(&skill_b_path).unwrap();
        fs::create_dir_all(&skill_c_path).unwrap();
        fs::write(skill_b_path.join("SKILL.md"), "# Planning B").unwrap();
        fs::write(skill_c_path.join("SKILL.md"), "# Planning C").unwrap();
        ...
        let graph = graph_with_deps(dir.path(), "source-a", vec!["source-b", "source-c"]);
        ...
        apply_test_renames(
            ...
            &renames,
            &graph,
            &["source-c".into(), "source-b".into()],
        );

        let rewritten = target.items["agents/coder.md"]
            .rewritten_content
            .as_ref()
            .expect("dependency agent should have been rewritten");
        let fm = crate::frontmatter::parse(rewritten).unwrap();
        assert_eq!(fm.skills(), vec!["planning__source-c"]);
    }

    #[test]
    fn pruned_same_source_unrenamed_item_allows_dependency_rewrite() {
        let dir = TempDir::new().unwrap();
        let agent_path = dir.path().join("source-a/agents/coder.md");
        fs::create_dir_all(agent_path.parent().unwrap()).unwrap();
        fs::write(&agent_path, "---\nskills: [planning]\n---\n# Agent\n").unwrap();
        ...
        let mut target = TargetState { items };
        let renames = vec![CollisionRename {
            original_name: "planning".into(),
            new_name: "planning__source-b".into(),
            source_name: "source-b".into(),
            kind: ItemKind::Skill,
        ...
        apply_test_renames(&mut target, &[], &renames, &graph, &["source-b".into()]);

        let rewritten = target.items["agents/coder.md"]
            .rewritten_content
            .as_ref()
            .expect("agent should have been rewritten after same-source item was pruned");
        let fm = crate::frontmatter::parse(rewritten).unwrap();
        assert_eq!(fm.skills(), vec!["planning__source-b"]);
    }
