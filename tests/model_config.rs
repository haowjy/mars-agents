...
#[test]
#[serial]
fn models_list_providers_and_no_visibility_conflict() {
    let server = MockServer::start();
    let (temp, project_root) = setup_project(&server);
...
#[test]
#[serial]
fn models_list_providers_blank_flag_shows_all() {
    let server = MockServer::start();
    let (temp, project_root) = setup_project(&server);
    let bin_dir = install_fake_harnesses(temp.path(), &["codex"]);
    fs::write(
...
"#,
    ...
    let mut cmd = mars_cmd(&project_root, temp.path(), &server.url(API_PATH));
    cmd.args(["--json", "models", "list", "--providers", "  "]);
    cmd.env("PATH", replace_path_with(&bin_dir));
...
#[test]
#[serial]
fn models_list_local_providers_override_project() {
    let server = MockServer::start();
    let (temp, project_root) = setup_project(&server);
    let bin_dir = install_fake_harnesses(temp.path(), &["codex"]);
    fs::write(
