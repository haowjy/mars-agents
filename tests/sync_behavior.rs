...
#[test]
fn sync_upgrade_hint_ignores_incompatible_transitive_tags() {
    let dir = TempDir::new().unwrap();
...
"#,
    ...
    add_tagged_release(
        &shared.repo_path,
        "v2.0.0",
        &[("agents/shared.md", "# Shared v2.0.0\n")],
    );
    ...
    mars()
        ...
        .assert()
        .success()
        .stderr(predicate::str::contains("mars upgrade").not());
}

#[test]
fn sync_upgrade_hint_refreshes_metadata_after_constraint_narrows() {
    let dir = TempDir::new().unwrap();
...
#[test]
fn sync_no_upgrade_hint_zeros_json_upgrade_count() {
    let dir = TempDir::new().unwrap();
...
"#,
    ...
    assert_eq!(parsed["upgrades_available"], json!(0));
}
